# -*- coding: utf-8 -*-
#
# Copyright © 2022, 2023, 2025, 2026 Genome Research Ltd. All rights reserved.
#
...
class TestChecksumUtilities:
    ...
    def test_checked_checksums_passes(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
    ...
    def test_check_checksums_failures(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
    ...
    def test_repair_checksums_all(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
        for p in obj_paths:
            ensure_common_metadata(DataObject(p))
    ...
    @m.it("Counts repairs correctly")
    def test_repair_checksums_none(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
...
class TestReplicaUtilities:
    ...
    @m.it("Returns false")
    def test_has_trimmable_replicas_valid(self, simple_data_object_path):
    ...
    @m.it("Returns true")
    def test_has_trimmable_replicas_valid_none(self, simple_data_object_path):
        obj = DataObject(simple_data_object_path)
    ...
    @m.it("Returns true")
    def test_has_trimmable_replicas_invalid(self, invalid_replica_data_object_path):
        obj = DataObject(invalid_replica_data_object_path)
    ...
    @m.it("Counts successes correctly")
    def test_checked_replicas_none(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
    ...
    @m.it("Counts failures correctly")
    def test_check_replicas_all(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
    ...
    @m.it("Counts repairs correctly")
    def test_repair_replicas_none(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
    ...
    @m.it("Counts repairs correctly")
    def test_repair_invalid_replicas_all(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
        for p in obj_paths:
    ...
    @m.it("Counts repairs correctly")
    def test_repair_valid_replicas_all(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
...
class TestConsentUtilities:
    ...
    def test_ensure_consent_withdrawn(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
    ...
    @m.it("Counts successes correctly")
    def test_checked_consent_withdrawn_passes(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
    ...
    @m.it("Counts failures correctly")
    def test_checked_consent_withdrawn_failures(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
    ...
    @m.it("Counts repairs correctly")
    def test_withdraw_consent_all(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
    ...
    @m.it("Counts repairs correctly")
    def test_withdraw_consent_none(self, annotated_collection_tree_path):
        obj_paths = collect_obj_paths(Collection(annotated_collection_tree_path))
...
class TestCopyUtilities:
    ...
    def test_copy_to_self(self, empty_collection_path, simple_data_object_path):
        c = Collection(empty_collection_path)
        with pytest.raises(ValueError):
            copy(c, c)

        d = DataObject(simple_data_object_path)
        with pytest.raises(ValueError):
            copy(d, d)
    ...
    def test_copy_collection(self, empty_collection_path):
        x = Collection(PurePath(empty_collection_path, "x"))
        x.create()
        assert x.exists()
        y = Collection(PurePath(empty_collection_path, "y"))
        y.create()
        assert y.exists()

        copy(x, y)
        assert Collection(PurePath(empty_collection_path, "y", "x")).exists()
    ...
    def test_copy_rename_collection(self, empty_collection_path):
        x = Collection(PurePath(empty_collection_path, "x"))
        z = Collection(PurePath(empty_collection_path, "x", "y", "z"))
        z.create(parents=True)
        assert z.exists()

        a = Collection(PurePath(empty_collection_path, "a"))
        assert not a.exists()
        assert Collection(a.path.parent).exists()

        copy(x, a, recurse=True)
        assert Collection(PurePath(empty_collection_path, "a", "y", "z")).exists()
    ...
    def test_copy_collection_error(self, empty_collection_path):
        x = Collection(PurePath(empty_collection_path, "x"))
        x.create()
        y = Collection(PurePath(empty_collection_path, "y"))
        y.create()
    ...
    @m.it("Creates a copy within the destination collection")
    def test_copy_data_object(self, empty_collection_path, simple_data_object_path):
        c = Collection(empty_collection_path)
        d = DataObject(simple_data_object_path)
        num_processed, num_copied = copy(d, c)

        assert DataObject(PurePath(c.path, simple_data_object_path.name)).exists()
        assert num_processed == 1
        assert num_copied == 1
    ...
    def test_copy_data_object_no_dest(
        self, empty_collection_path, simple_data_object_path
    ):
        c = Collection(empty_collection_path)
        d = DataObject(simple_data_object_path)
    ...
    @m.it("Raises an exception, unless exists_ok is True")
    def test_copy_data_object_error(
        self, empty_collection_path, simple_data_object_path
    ):
        x = Collection(PurePath(empty_collection_path, "x"))
        x.create()
    ...
    def test_copy_recurse(self, annotated_collection_tree_path, empty_collection_path):
        src = Collection(annotated_collection_tree_path)
        dest = Collection(empty_collection_path)

        num_processed, num_copied = copy(src, dest, recurse=True)

        expected = [
            PurePath(dest.path, annotated_collection_tree_path.name, p).as_posix()
            for p in [
                "./",
                "./a",
                "./a/m",
    ...
    def test_copy_annotation_recurse(
        self, annotated_collection_tree_path, empty_collection_path
    ):
        src = Collection(annotated_collection_tree_path)
        dest = Collection(empty_collection_path)
        ...
        expected_root = PurePath(dest, src.path.name)
        ...
        for item in dest.contents(avu=True, recurse=True):
            ...
            relative_part = PurePath(str(item)).relative_to(expected_root)
            item_src = src.path / relative_part
            assert item.metadata() == [AVU("path", item_src.as_posix())]
...
class TestSafeRemoveUtilities:
    ...
    def test_write_safe_remove_commands(self, annotated_collection_tree_path):
        with StringIO() as writer:
            write_safe_remove_commands(annotated_collection_tree_path, writer)
            ...
            expected = [
                ...
                ("irmdir", annotated_collection_tree_path.name),  # The root
            ]
            observed = []
    ...
    def test_write_safe_remove_script(self, tmp_path, annotated_collection_tree_path):
        script = Path(tmp_path, "safe_rm.sh")
        with open(script, "w", encoding="UTF-8") as writer:
            write_safe_remove_script(writer, annotated_collection_tree_path)
        subprocess.run([script.as_posix()], check=True)

        assert not Collection(annotated_collection_tree_path).exists()
    ...
    @m.it("Writes the expected commands")
    def test_write_safe_remove_commands_special(self, challenging_irods_paths):
        with StringIO() as writer:
            write_safe_remove_commands(challenging_irods_paths, writer)
    ...
    @m.it("Removes the expected collections and data objects")
    def test_write_safe_remove_script_special(self, tmp_path, challenging_irods_paths):
        script = Path(tmp_path, "safe_rm.sh")
        with open(script, "w", encoding="UTF-8") as writer:
            write_safe_remove_script(writer, challenging_irods_paths)
        subprocess.run([script.as_posix()], check=True)

        assert not Collection(challenging_irods_paths).exists()
...
class TestMetadataUtilities:
    ...
    def test_update_secondary_metadata(
        ...
        obj_path = single_study_single_sample_data_object_path.as_posix()
